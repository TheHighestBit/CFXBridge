...
tar -czf CFXBridge-1.0.5-linux-x64.tar.gz -C publish-linux-x64 .
```

```bash
dotnet publish -c Release -r linux-arm --self-contained false -o publish-linux-arm
tar -czf CFXBridge-1.0.5-linux-arm.tar.gz -C publish-linux-arm .
```

```bash
dotnet publish -c Release -r linux-arm64 --self-contained false -o publish-linux-arm64
tar -czf CFXBridge-1.0.5-linux-arm64.tar.gz -C publish-linux-arm64 .
```

```bash
dotnet publish -c Release -r win-x64 --self-contained false -o publish-win-x64
tar -czf CFXBridge-1.0.5-win-x64.tar.gz -C publish-win-x64 .
```